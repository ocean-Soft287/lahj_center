...
class AddAdvertismentServiceLocator {
  static Future<void> execute({required GetIt getIt}) async {
    getIt.registerFactory<CategoryBloc>(()=> CategoryBloc(getIt<Addadvertisminterepo>())..add(GetCategories()));
    getIt.registerFactory<GovernmentBloc>(()=> GovernmentBloc(getIt<Addadvertisminterepo>())..add(GetGovernments()));
    getIt.registerFactory<ServicesBloc>(()=> ServicesBloc(getIt<Addadvertisminterepo>())..add(GetServices()));
    getIt.registerFactory<CurrencyBloc>(()=> CurrencyBloc(getIt<Addadvertisminterepo>())..add(GetCurrencies()));
    getIt.registerFactory<AddAdvertisementBloc>(()=> AddAdvertisementBloc(getIt<Addadvertisminterepo>()));
  }
}